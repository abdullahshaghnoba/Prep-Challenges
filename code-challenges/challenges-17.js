...
const filterLinks = (str) => {
    // write your code here
    const match = str.match(/((www|ftp)\.)[-A-Z0-9+&@#\/%?=~_|$!:,.;]*[A-Z0-9+&@#\/%=~_|$]/ig);
...
const isPalindrome = (str) => {
    ...
    const regex = /\W/ig;
    let newStr = lowerCaseStr.replaceAll(regex, '');
...
const samePattern = (str, arr) => {
    ...
    let reversedArr = [];
    arr.map((e) => { reversedArr.unshift(e); });
    let newArr = arr.toString()
    let newReversedArr = reversedArr.toString()
    if (str == reversedStr && newArr == newReversedArr) {
        return true;
    } else if (str != reversedStr && newArr != newReversedArr) {
        return true;
    } else {
        return false;
    }
